...
!!! note
    This behavior applies to all versions of {{pcsm.short}}.
...
=== "Replica Sets"
    ...
    1. **Set up authentication**: Create users for {{pcsm.short}} in both MongoDB deployments. Configure MongoDB connection string URIs for your source and target that list all replica set members, then start {{pcsm.short}} using these user credentials. See [Configure authentication in MongoDB](install/authentication.md) for details.
    ...
    2. **Start the replication**
        ...
        !!! info "Important"
            ...
            - Existing databases and collections on the target that are not selected for replication **remain untouched**.
...
=== "Sharded Clusters (Tech Preview)"
    ...
    2. **Shard key setup during `start` (if applicable)**

        When you run `start`, if the source collection is sharded, {{pcsm.short}} creates the corresponding sharded collection on the target using the same shard key; the target cluster manages other sharding details internally.
    ...
    3. **Start the replication**
        ...
        Call the `start` command. You don't have to disable the balancer on the target. During startup, {{pcsm.short}} ensures that sharded collections selected for replication exist on the target with the same shard key as the source, then it starts copying all data from the source to the target. First it does the initial sync by cloning the data and then applying all the changes that happened since the clone start. See [Start the replication](install/usage.md#start-the-replication) for command details.

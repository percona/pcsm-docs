...
1. **Set up authentication**: Create users for {{pcsm.short}} in both MongoDB deployments. Start and connect {{pcsm.short}} to your source and target using these user credentials and the replica set members' hostnames and ports. See [Configure authentication in MongoDB](install/authentication.md) for details.

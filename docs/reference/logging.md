...
Logging is configured when the `plm` server starts. The following flags are available:
...
**Example:** To see detailed debugging messages, start `plm` with:
```sh
...
You can disable the colorization with the `--no-color` flag, which is useful when redirecting log output to a file.

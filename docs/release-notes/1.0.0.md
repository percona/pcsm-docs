...
* [PCSM-369](https://perconadev.atlassian.net/browse/PCSM-369): Restricted the `--mongodb-operation-timeout` flag to the `server` and `reset` commands, which create MongoDB clients. Previously, other commands accepted the flag but silently ignored its value. The `start`, `status`, `pause`, `resume`, and `finalize` commands now return an unknown flag error when this flag is provided.
* [PCSM-367](https://perconadev.atlassian.net/browse/PCSM-367): Fixed reported lag that dropped to zero when the initial clone finished. An empty change-stream batch carried the current timestamp and reset the lag calculation. An empty batch no longer counts as applied progress.

...
PCSM now supports active-standby high availability during replication. You can run two or more instances of PCSM with the same source and target. While one of them is actively replicating, the others are in standby mode.
...
* [PCSM-328](https://perconadev.atlassian.net/browse/PCSM-328): Improved migration reliability by retrying MongoDB operations that fail because of transient errors, such as brief network interruptions or timeouts. Previously, a temporary source-side failure could stop a long-running initial data copy.
...
* [PCSM-367](https://perconadev.atlassian.net/browse/PCSM-367): Fixed an issue where the reported replication lag dropped to zero after the initial clone completed. An empty change-stream batch used the current timestamp and incorrectly reset the lag calculation. This could cause `initialSync.completed` to evaluate to true before catch-up finished, potentially resulting in data loss. PCSM no longer treats an empty batch as replication progress.

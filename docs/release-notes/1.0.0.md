...
We’re excited to announce {{ pcsm.full_name }} (PCSM) 1.0.0. This release brings sharding support to general availability, adds active-standby high availability during replication, and expands the supported migration topologies. You can also run multiple PCSM instances against the same source cluster, with each instance synchronizing a different subset of data to a separate target.
...
Sharding support reaches general availability (GA) in PCSM 1.0.0. PCSM supports migrations from a sharded source cluster to another sharded cluster or to a replica set target.
...
For more information, see the [documentation](https://docs.percona.com/percona-clustersync-for-mongodb/sharding.html).

### High availability during replication

PCSM now supports active-standby high availability during replication. For example you can run two or more instances of PCSM with the same source and target. While one of them is actively replicating the others are in standby mode. 

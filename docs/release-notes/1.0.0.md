...
!!! warning "Upgrading from an earlier version requires a reset"
    ...
    **PCSM 1.0.0** cannot use replication state created by an earlier PCSM version. Before upgrading, stop all instances of the earlier version and [reset the stored replication state](../pcsm-commands.md#reset) on the target cluster.
